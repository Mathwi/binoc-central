...
var DownloadsTaskbar = {
  ...
  attachIndicator(aWindow) {
    ...
    if (this._taskbarProgress) {
      this._taskbarProgress.setProgressState(Components.interfaces.nsITaskbarProgress.STATE_NO_PROGRESS);
    }
    ...
    let docShell = aWindow.QueryInterface(Components.interfaces.nsIInterfaceRequestor)
                          ...
                          .getInterface(Components.interfaces.nsIXULWindow).docShell;
    ...
    aWindow.addEventListener("unload", () => {
      ...
      if (windows.hasMoreElements()) {
        newActiveWindow = windows.getNext().QueryInterface(Components.interfaces.nsIDOMWindow);
      }
      if (newActiveWindow) {
  ...
  onSummaryChanged() {
    ...
    if (this._summary.allHaveStopped || this._summary.progressTotalBytes == 0) {
      this._taskbarProgress.setProgressState(
                               Components.interfaces.nsITaskbarProgress.STATE_NO_PROGRESS, 0, 0);
    } else {
      ...
      this._taskbarProgress.setProgressState(
                               Components.interfaces.nsITaskbarProgress.STATE_NORMAL,
